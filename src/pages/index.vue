...
export default {
  ...
  async asyncData({app, error}) {
    try {
      const { data } = await app.$graphqlClient.fetch(GET_TODOS);
      return {
        todos: data.allTodos,
      };
    }
  ...
  methods: {
    async getTodos() {
      const { data } = await this.$graphqlClient.fetch(GET_TODOS);
    ...
    async addTodo() {
      ...
      // Add todo to list
      await this.$graphqlClient.fetch(ADD_TODO, { name: this.newTodo });
    ...
    async removeTodo(id) {
      await this.$graphqlClient.fetch(REMOVE_TODO, { id });
      // Update the todo list
      this.getTodos();
    },
  },
